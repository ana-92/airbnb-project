...
class SignUp extends Component {
    ...
    submitLogin = async (e) => {
        ...
        } else if (response.msg === "invalidData") {
            swal({
                title: "Invalid email/password",
                text: "Please provide a valid email and password",
                icon: "error",
            })
        } else if (response.msg === "userAdded") {
            swal({
                title: "Success!",
                icon: "success",
            });
        ...
        return (
            <div className="login-form">
                <form onSubmit={this.submitLogin}>
                    ...
                    <div>Already have an account? <span className="pointer" onClick={() => { this.props.openModal(true, <Login />) }}>Log in</span></div>
                </form>
            </div>
